...
export async function forceImportStore(dbId, jsonStr) {
    try {
        ...
        if (dbJson === null ||
            ...
            throw new Error("The store dump is missing its format marker and cannot be imported.");
        }
        if (dbJson.formatVersion !== STORE_DUMP_FORMAT_VERSION) {
            throw new Error(`Unsupported store dump format version: ${dbJson.formatVersion}`);
        }
        if (dbJson.tables === null ||
        ...
        for (const tableName of REQUIRED_FOREST_TABLES) {
            if (!Array.isArray(dbJson.tables[tableName])) {
                throw new Error(`The store dump is missing required forest table "${tableName}".`);
            }
        }
        ...
        for (const tableName of jsonTableNames) {
            ...
            transformedTables[tableName] = await Promise.all(records.map(transformForImport));
        }
