    ...
    return await db.dexie.transaction("rw", tablesToAccess, async (tx) => {
        await Promise.all([
            Promise.all(serializedInputNotes.map((note) => {
                return upsertInputNote(dbId, note.noteId, note.noteAssets, note.attachments, note.serialNumber, note.inputs, note.noteScriptRoot, note.noteScript, note.nullifier, note.createdAt, note.stateDiscriminant, note.state, note.consumedBlockHeight, note.consumedTxOrder, note.consumerAccountId);
            })),
