...
describe("sync", () => {
  ...
  describe("applyStateSync — partial blockchain nodes", () => {
    ...
    it("accepts an existing partial blockchain node with the same value", async () => {
      const dbId = await openTestDb();
      ...
      expect(node!.node).toBe("node-data");
    });

    it("rejects a conflicting partial blockchain node", async () => {
      const dbId = await openTestDb();
      ...
      ).rejects.toThrow("Conflicting partial blockchain node at index 10");
