...
* [BREAKING][api][web] Account "kind" predicates were removed (they no longer exist in protocol 0.15): `Account`, `AccountId`, and `FetchedAccount` no longer expose `isFaucet()`, `isRegularAccount()`, `isNetwork()`, or `isUpdatable()`. `isPublic()`/`isPrivate()`/`isNew()` remain. Faucet-vs-wallet is now a component/role concept rather than an account type. ([#NN](https://github.com/0xMiden/web-sdk/pull/NN))
* [BREAKING][api][web] `AccountType` now has only `Private`/`Public` (account visibility); `StorageMode`/`AccountStorageMode` dropped `Network` (network accounts were removed from the protocol). ([#NN](https://github.com/0xMiden/web-sdk/pull/NN))
* [BREAKING][api][web,react] Account code mutability was removed (it no longer exists in protocol 0.15). `newWallet(...)` and `importPublicAccountFromSeed(...)` no longer take a `mutable` flag, and the React `useCreateWallet` / `useImportAccount` / `useSessionAccount` hooks dropped their `mutable` option. `accounts.create({ type })` no longer accepts the wallet/contract mutability values, and the `AccountType` const now exposes only `FungibleFaucet` / `NonFungibleFaucet` (a wallet is the default, a contract is any `create()` call that passes `components`). ([#NN](https://github.com/0xMiden/web-sdk/pull/NN))
* [BREAKING][behavior][web] `newFaucet(...)` builds faucets from the protocol 0.15 `FungibleFaucet` component (symbol/decimals/max-supply) plus a `TokenPolicyManager` (allow-all mint/burn). `BasicFungibleFaucetComponent.fromAccount(...)` reads that component, so faucets minted by pre-0.15 SDKs can't be introspected through it. ([#NN](https://github.com/0xMiden/web-sdk/pull/NN))
* [BREAKING][api][web] `NoteAttachment` was redesigned: construct with `NoteAttachment.withWord(scheme, word)` / `withWords(scheme, words)` (replacing `newWord`/`newArray`, which took felts) and read content with `asWords()` (replacing `asWord()`/`asArray()`). `NoteAttachmentScheme` is now a `u16` — use `asU16()` (was `asU32()`) and note its constructor now throws on out-of-range values. `NoteMetadata` no longer exposes `attachment()`/`withAttachment()`. ([#NN](https://github.com/0xMiden/web-sdk/pull/NN))
* [BREAKING][api][web] Note id/commitment surface changed: `NoteHeader.toCommitment()` → `metadataCommitment()`; `NoteDetails.id()` → `detailsCommitment()` (returns a `Word`) and `NoteDetails.nullifier()` was removed (a nullifier requires metadata); `new NoteId(detailsCommitment, metadata)` replaces `new NoteId(recipient, assetCommitment)`. `FetchedNote` now exposes `noteId` and `metadata` directly (the `header` getter was removed). ([#NN](https://github.com/0xMiden/web-sdk/pull/NN))
* [BREAKING][api][web] `InputNoteRecord.id()` now returns `NoteId | undefined` — metadata-less expected notes have no id, so use `detailsCommitment` as a stable key. `NoteFile.noteId()`/`nullifier()` are likewise optional. ([#NN](https://github.com/0xMiden/web-sdk/pull/NN))
* [BREAKING][behavior][web] `importNoteFile(...)` now resolves to the note's details-commitment `Word` (was a `NoteId`). `Note.createP2IDNote`/`createP2IDENote` take an optional attachment (pass `undefined` for none). The `Felt` constructor now throws when the value does not fit in the base field. ([#NN](https://github.com/0xMiden/web-sdk/pull/NN))
* [BREAKING][api][react] The send hooks and note utilities follow the new attachment API; `readNoteAttachment(...)` returns `null` because protocol 0.15 no longer carries attachment content on note metadata. ([#NN](https://github.com/0xMiden/web-sdk/pull/NN))
