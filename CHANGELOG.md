...
* [FIX][web] `js_u64_to_u64` (the JsU64→u64 conversion behind every JS-facing `bigint` parameter - `Felt`, `Word`, `FungibleAsset`, `TransactionStatus.committed`, `TransactionProver.newRemoteProver`, and every mint/send/swap transaction request builder) no longer panics the WASM/native module on a negative or >=2^64 BigInt from JS. Now returns a catchable error at each of those ~20 call sites; `MerklePath`/`SparseMerklePath`'s `verify` return `false` instead, matching their existing behavior for any other invalid input ([web-sdk#341](https://github.com/0xMiden/web-sdk/pull/341)).
