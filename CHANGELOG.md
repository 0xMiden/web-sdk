...
* [CHORE][ci] Validate the consolidated trusted-publishing flow end-to-end (`pnpm publish` + npm OIDC + `--provenance`) by cutting `0.15.0-alpha.4` to the `next` dist-tag. Earlier attempts: `alpha.1` died on a missing executable bit (fixed in `alpha.2`); `alpha.2` was rejected by npm's provenance verifier (Warp self-hosted runner, fixed in `alpha.3` by moving publish to `ubuntu-24.04`); `alpha.3` published the three Node.js native packages but the web-client + react-sdk publishes were rejected with `Failed to validate repository information: package.json: "repository.url" is ""` — `crates/web-client/package.json` was missing a `repository` field. `alpha.4` adds it.
