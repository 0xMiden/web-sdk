...
* [PERF][web] **WASM SIMD (`+simd128`) on the MT build with LLVM auto-vectorization disabled.** Earlier blanket `+simd128` regressed prove time 16-37% because LLVM's autovectorizer mis-vectorized Goldilocks's u64 modular reduction (WASM v128 has no widening 64×64 multiply, so the emulated shift+add chains cost more than the scalar `mul` they replaced). Re-enabled paired with `-C llvm-args=-vectorize-loops=false -vectorize-slp=false` so hand-written WASM-SIMD paths in libraries (BLAKE3 etc.) light up via `cfg(target_feature = "simd128")` while Goldilocks scalar code stays scalar. Measured median improvement of ~5-7% on ECDSA prove, larger on Falcon. WASM artifact size unchanged.
* [PERF][build] **Patched `@wasm-tool/rollup-plugin-rust@3.1.5` to drop `-Z build-std-features=optimize_for_size`.** The plugin unconditionally appends that flag to the cargo invocation when building in release+rustc-optimize mode. It rebuilds std with size-priority paths (simpler sort, smaller memcpy, weaker HashMap hash) — the exact opposite of what a STARK prover needs. Empirical regression: ECDSA-MT send goes from ~3.5s (without the flag) to ~12s (with it). Patched via `pnpm.patchedDependencies`. Auto-applied on every install. ([review thread on PR #134](https://github.com/0xMiden/web-sdk/pull/134) for context.)
* [BUILD] **Pinned `rust-toolchain.toml` to `nightly-2026-05-05`** so published WASM bytes and prove perf are reproducible across rebuilds. Bump deliberately, never via floating `nightly`.
