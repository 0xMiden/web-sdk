...
* [FEATURE][web] Added `MidenClient.rawWebClient()`, a supported accessor that returns the underlying `WasmWebClient` the client already owns. Advanced consumers (e.g. `@openzeppelin/miden-multisig-client`) that need low-level operations can now **borrow** this instance instead of reconstructing a second `WasmWebClient` over the same store — two live clients over one IndexedDB database don't share in-memory state, so notes synced through one were not visible through the other (the "synced notes not found" dual-instance bug). This is the supported borrow hatch #169 calls for; together with `compile.component({ libraries })` and `notes.listConsumable` above, consumers can now run entirely on the single client a `MidenClient` owns. ([#170](https://github.com/0xMiden/web-sdk/pull/170)).

### Changes

* [BREAKING][behavior][web] `notes.listAvailable({ account })` now returns only notes consumable **right now**, excluding block-locked notes (status `consumableAfterBlock`) it previously included — the name implied immediate usability, but callers could receive notes the chain would reject. Use `notes.listConsumable()` for the full set (now + block-locked) with consumability metadata. ([#170](https://github.com/0xMiden/web-sdk/pull/170)).
