...
CI step `Check that web client documentation is up-to-date` (in `.github/workflows/test.yml`) runs `pnpm --filter @miden-sdk/miden-sdk run typedoc` then `git diff --exit-code docs/typedoc/web-client`. **Any change to the curated public surface — i.e. anything reachable from [`crates/web-client/js/types/docs-entry.d.ts`](crates/web-client/js/types/docs-entry.d.ts) (which re-exports `api-types.d.ts` wholesale plus selected WASM classes) — requires regenerating typedoc and committing the diff.** If you don't, CI blocks merge. The right workflow is:

```bash
pnpm --filter @miden-sdk/miden-sdk run build      # need dist/ for typedoc inputs
pnpm --filter @miden-sdk/miden-sdk run typedoc    # regenerate docs/typedoc/web-client/
git add docs/typedoc/web-client && git commit
```

If you forget, the failing CI message will tell you which file drifted.
...
| `docs/typedoc/web-client/` (generated) | Re-run `pnpm --filter @miden-sdk/miden-sdk run typedoc` and commit the regenerated tree. CI fails the build if this is stale. | Any change reachable from `docs-entry.d.ts`. |
...
- **Regenerate typedoc.** If you touched anything reachable from `docs-entry.d.ts`, run `pnpm --filter @miden-sdk/miden-sdk run typedoc` and commit. CI gates on this.
