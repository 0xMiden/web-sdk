...
describe("useMint", () => {
  ...
  describe("non-Error catch branch (lines 110-114)", () => {
    it("should wrap non-Error thrown values in an Error", async () => {
      const mockClient = createMockWebClient({
        newMintTransactionRequest: vi
          .fn()
          .mockReturnValue(createMockTransactionRequest()),
        submitNewTransaction: vi.fn().mockRejectedValue("string mint error"),
      });
      ...
      await act(async () => {
        ...
        ).rejects.toThrow("string mint error");
      });

      await waitFor(() => {
        expect(result.current.error?.message).toBe("string mint error");
      });
    });
  });

  describe("prover path", () => {
    it("should use submitNewTransactionWithProver when prover is set (lines 93-97)", async () => {
      const mockTxId = createMockTransactionId("0xtxmintprover");
