...
const loadTurnkeySignerProvider = (mocks = {}) => {
  ...
  Module._load = function patchedLoad(request, parent, isMain) {
    ...
    if (request.startsWith("@miden-sdk/turnkey")) {
      return mocks["@miden-sdk/turnkey"];
    }
  ...
  try {
    const filePath = path.resolve(
      __dirname,
      "../../react/src/TurnkeySignerProvider.tsx"
    );
...
const defaultConfig = {
  ...
  stamper: {
    stamp: async () => ({ stampHeaderName: "x", stampHeaderValue: "y" }),
  },
};
...
const renderHookInProvider = async (
  ...
  return {
    ...
    rerender: async (newProps) => {
      await act(async () => {
        testRenderer.update(
          React.createElement(TurnkeySignerProvider, {
            ...defaultProps,
            ...newProps,
          })
        );
...
test("TurnkeySignerProvider renders children", async () => {
  ...
  try {
    ...
    const { testRenderer } = await renderProvider(
      TurnkeySignerProvider,
      mocks,
      {
...
test("isConnected is false initially", async () => {
  ...
  try {
    ...
    assert.strictEqual(
      result.account,
      null,
      "Account should be null initially"
    );
  } finally {
    restore();
  }
...
test("setAccount() updates connection state", async () => {
  ...
  try {
    ...
    assert.strictEqual(
      ...
      true,
      "Should be connected after setAccount"
    );
