...
const cliPath = path.resolve(
  __dirname,
  "../../create/bin/create-miden-para-react.mjs"
);
...
test("CLI scaffolds template and patches package.json in test mode", () => {
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "miden-para-create-"));
  ...
  assert.ok(
    pkg.dependencies["@miden-sdk/para"],
  ...
  assert.match(appContents, /@miden-sdk\/para-react/);
