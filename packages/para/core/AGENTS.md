...
- **Root package (`@miden-sdk/para`)** — exports `createParaMidenClient`, low-level helpers (`signCb`, modal helpers, key/seed utilities) and type definitions. Builds ship as dual CJS/ESM bundles generated by `scripts/build.mjs`.
- **React hook (`packages/use-miden-para-react`, published as `@miden-sdk/para-react`)** — `useParaMiden` watches Para React SDK state, instantiates `createParaMidenClient`, and hands back `{ client, accountId, para, evmWallets }` for UI code.
...
3. **React integration (`packages/use-miden-para-react/src/useParaMiden.ts`)**
   ...
   - Imports the root npm package (`@miden-sdk/para`).
4. **Example consumer (`examples/react/src/components/ConsumeAllNotes.tsx`)**
   ...
   - Imports both the root npm package (`@miden-sdk/para`) and the react integration (`@miden-sdk/para-react`).
